...
class TextAnalyzer:
    ...
    async def analyze_text(self, page_number: int, page_data: Dict[str, Any], usage_tracker: LLMUsageTracker = None) -> Dict[str, Any]:
        """
        Analyze page by extracting text from image first
        """
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                print(f"    [!] JSON error on page {page_number} (attempt {attempt}): {e}")
    ...
    def _create_prompt(self, page_number: int) -> str:
        """
        Create detailed prompt for Gemini Vision to extract and analyze from image
        NO HALLUCINATED HEADINGS - NO FAKE IDs - NO BROKEN ADDRESSES
        """
        
        prompt = f"""You are a precision document extraction system. Extract EXACTLY what is in the PDF page image.
...
3. **Handle Bullet Lists and Enumerations:**
   ...
   ✓ Example: If text shows:
      ...
      • Experian
      • K-1 Analyzer
     Then extract as content under one heading, NOT separate sub-headings
   ✓ Preserve exact bullet formatting
   ✗ NEVER merge multiple bullets into one

4. **Preserve Numeric Values EXACTLY:**
   ✓ Extract numbers, Amount, Price, IDs, order codes EXACTLY as printed
...
NOW: Extract ALL text from the image and analyze it following ALL rules above.
...
Return ONLY the JSON object."""
    ...
    def _empty_result(self, page_number: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "sections": [],
            "sections_count": 0,
            "summary": f"Page {page_number}: Analysis failed.",
            "entities": {
                "document_type": None,
                "buyer_name": None,
                "seller_name": None,
